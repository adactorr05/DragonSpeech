...
/**
 * Goal for a dragon to follow its owner - mostly vanilla's own
 * FollowOwnerGoal, with a fix for a real issue plus letting the dragon
 * teleport to its owner while airborne so it doesn't get stuck
 * following on the ground until the owner lands. Ported near-verbatim
 * from Dragon Mounts Legacy
 * (com.github.kay9.dragonmounts.dragon.ai.DragonFollowOwnerGoal,
 * GPL-3.0, original author credit: AnimalsWritingCode).
 *
 * Uses dragon.getOwner() (vanilla TamableAnimal's own owner field)
 * directly, no adaptation needed - DragonEntity.setBondedOwner() keeps
 * that field in sync with this project's own bonding data specifically
 * so goals like this one (and anything else inherited from
 * TamableAnimal that reads getOwner()) keep working unmodified.
 */
public class DragonFollowOwnerGoal extends Goal {
    ...
    public DragonFollowOwnerGoal(DragonEntity dragon, double speedModifier, float startDistance, float stopDistance, float teleportDistance) {
        this.dragon = dragon;
    ...
    public boolean canUse() {
        // FIX: "the dragon seems to follow you even if the follow you
        // isn't on" per explicit direction - real bug, confirmed
        // directly: this goal never checked optionFollowing() at all,
        // only isOrderedToSit() - the follow behavior ran regardless of
        // the toggle.
        if (!dragon.optionFollowing()) {
            return false;
        }
